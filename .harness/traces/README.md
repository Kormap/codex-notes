...
Trace를 생성·갱신하기 전에는 [실행 계약](runtime.md)을 반드시 읽는다. 이 README는 schema·enum 변경, 상세 해석, 보관·공유·report source 선택 시에 읽는다.
...
기록 시점, 재시도와 독립 실행의 구분, ID·파일 이름과 시간 규칙은 [실행 계약](runtime.md#기록과-판정)을 따른다.
...
유출 시 중단·대응은 [실행 계약](runtime.md#저장과-민감정보)을 따른다. 공유 시에는 문맥상 재식별 가능성도 검토한다.
