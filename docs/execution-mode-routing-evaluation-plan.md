...
| `EV-ROUTING-03` | `AC-ROUTING-03` | `REQUIRED` | assignment의 writer scope 포함·중첩과 실행 전후 actual changed files exact 목록을 각각 대조 |
...
`EV-ROUTING-03`은 exact 파일과 `<directory>/**` subtree scope를 허용한다. scope는 정규화된 workspace 상대 POSIX path segment로 판정하며 절대 경로, `..`, 비정규 경로와 `/**` 이외 glob을 거부한다. 모든 actual changed file은 정확히 한 writer scope에 포함되어야 하고, 서로 다른 writer의 exact 중복과 exact/subtree 또는 부모·자식 subtree 중첩은 validator가 직접 계산한다. `writer_overlap` 자기신고는 이 계산을 대체하지 않는다. 유사 문자열 prefix는 path segment가 다르면 중첩이 아니다.

SCN-05의 고정 writer scope는 `fixtures/module-a/**`, `fixtures/module-b/**`이고, actual changed files exact 기대 목록은 `fixtures/module-a/module.mjs`, `fixtures/module-b/module.mjs`다. 두 목록은 서로 다른 계약으로 검증한다.
